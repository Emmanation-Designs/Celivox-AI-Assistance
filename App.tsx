...
export default function App() {
  ...
  const handleNewChat = async () => {
    ...
    try {
      ...
      if (newRef.key) {
        ...
        // Ensure menu is closed for new chat
  ...
  const handleRenameTopic = async (e: React.FormEvent) => {
    ...
    try {
        ...
        setEditingTopicId(null);
        // HIDE MENU AFTER RENAME (Goes inside again)
        setActiveTopicMenuId(null);
    } catch(e) {
  ...
  const handleDeleteTopic = async (topicId: string, e: React.MouseEvent) => {
    ...
    try {
        ...
        // Menu is naturally hidden as topic is removed
    } catch(e) {
  ...
  const handleDelete = async (fullId: string, e: React.MouseEvent) => {
    ...
    // Safely split by LAST underscore to handle IDs containing underscores
    const lastUnderscoreIndex = fullId.lastIndexOf('_');
    if (lastUnderscoreIndex === -1) {
        console.error("Invalid ID format (no underscore)", fullId);
        return;
    }
  ...
  const startListeningLoop = () => {
    ...
    try {
      ...
      recognition.onend = () => {
         recognitionRef.current = null;
         if (isLiveModeRef.current && !isProcessingRef.current) {
  ...
  const handleSend = async (textOverride?: string, options: { isLive?: boolean } = {}) => {
    ...
    // GENERATE FIREBASE PUSH KEY UPFRONT FOR SYNC
    const validUid = uid;
    ...
    try {
        ...
        // Create the ref early so we have the KEY
    ...
    const firebaseKey = newMessageRef.key!; // Guaranteed key

    const optimisticUserMsg: Message = {
      id: firebaseKey + '_q', // SYNCED ID
      role: 'user',
      text: textToSend,
    ...
    try {
      ...
      // Firebase Save using the PRE-GENERATED Ref
      try {
